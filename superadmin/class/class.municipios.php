...
class municipios
{
	...
	public function getnommuni($dpto)
	{
		try
		{
			$query=$this->conex->prepare("SELECT * FROM municipios WHERE Nombre_Departamento=?");
			$query->bindParam(1, $dpto);
			$query->execute();
			$y="";
			while($row=$query->fetchObject())
			{
				$y=$y."<option value='".$row->Nombre_Municipio."'>".$row->Nombre_Municipio."</option>";
			}
			$this->conex=null;
			echo $y;
		}
